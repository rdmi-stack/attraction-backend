...
const generateTicketPdf = async (data) => {
    return new Promise(async (resolve, reject) => {
        try {
            ...
            doc.on('error', reject);
            // Generate QR Code
            const qrCodeDataUrl = await qrcode_1.default.toDataURL(data.reference, {
                width: 150,
                margin: 1,
            });
            ...
            data.items.forEach((item) => {
                const guests = [];
                if (item.quantities.adults > 0)
                    guests.push(`${item.quantities.adults} Adult(s)`);
                if (item.quantities.children > 0)
                    guests.push(`${item.quantities.children} Child(ren)`);
                if (item.quantities.infants > 0)
                    guests.push(`${item.quantities.infants} Infant(s)`);
                doc
                    .fillColor(textColor)
                    .fontSize(12)
                    .font('Helvetica')
            ...
            doc
                .fillColor(mutedColor)
                .fontSize(9)
                .text('Please show this e-ticket on your mobile device at the venue. No printing required.', 50, 750, { align: 'center', width: 495 });
            doc
                .fillColor(mutedColor)
                .fontSize(8)
                .text(`Generated on ${new Date().toISOString().split('T')[0]} | Powered by Attractions Network`, 50, 770, { align: 'center', width: 495 });
            doc.end();
        }
        catch (error) {
            reject(error);
